...
int main()
{
    ...
    // Staging buffer for our terrain data that we will upload to our
    // VertexBuffer
        ...
        while (window.pollEvent(event))
        {
            ...
            if (prerequisitesSupported && (event.type == sf::Event::KeyPressed))
            {
                switch (event.key.code)
                {
                    ...
                    case sf::Keyboard::Up:
                        currentSetting =
                            (currentSetting + settingCount - 1) % settingCount;
                        break;
        ...
        {
            {
                ...
                // Don't bother updating/drawing the VertexBuffer while terrain
                // is being regenerated
                if (!pendingWorkCount)
                {
                    // If there is new data pending to be uploaded to the
                    // VertexBuffer, do it now
...
sf::Color getLowlandsTerrainColor(float moisture)
{
    sf::Color color =
        ...
        : moisture < 0.48f
            ? sf::Color(0, 200 - 40 * (moisture - 0.4f) / 0.08f, 0)
                     ...
                     const WorkItem& workItem)
{
    ...
    if (rowStart >= resolutionY) return;
    ...
    for (unsigned int y = rowStart; y < rowEnd; y++)
    {
        for (int x = 0; x < resolutionX; x++)
        {
            ...
            else
            {
                ...
                vertices[arrayIndexBase + 0].texCoords = computeNormal(
                    x, y, getElevation(x - 1, y), getElevation(x + 1, y),
            ...
            else
            {
                ...
                vertices[arrayIndexBase + 1].texCoords =
                    computeNormal(x, y + 1, getElevation(x - 1, y + 1),
            ...
            vertices[arrayIndexBase + 2].texCoords = computeNormal(
                x + 1, y + 1, getElevation(x, y + 1),
                getElevation(x + 2, y + 1), getElevation(x + 1, y + 2),
                getElevation(x + 1, y));
