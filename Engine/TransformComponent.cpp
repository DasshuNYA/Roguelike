...
namespace Engine
{
	...
	void TransformComponent::SetParent(TransformComponent* newParent)
	{
		...
		else if (parent != nullptr && newParent != nullptr)
		{
			...
			localTransform =
				newParent->GetWorldTransform().GetInversed()
	...
	void TransformComponent::setWorldInfoFrom(const Matrix2D& transform) const
	{
		...
		scale.x = std::sqrtf(
			transform.GetMatrix()[0][0] * transform.GetMatrix()[0][0]
			+ transform.GetMatrix()[1][0] * transform.GetMatrix()[1][0]
		);

		scale.y = std::sqrtf(
			transform.GetMatrix()[0][1] * transform.GetMatrix()[0][1]
	...
	void TransformComponent::setLocalInfoFrom(const Matrix2D& transform) const
	{
		...
		localScale.x = std::sqrtf(
			transform.GetMatrix()[0][0] * transform.GetMatrix()[0][0]
			+ transform.GetMatrix()[1][0] * transform.GetMatrix()[1][0]
		);

		localScale.y = std::sqrtf(
			transform.GetMatrix()[0][1] * transform.GetMatrix()[0][1]
			+ transform.GetMatrix()[1][1] * transform.GetMatrix()[1][1]
		);
	...
	Matrix2D TransformComponent::createTransform(
		const Vector2Df& position,
		float rotation,
		const Vector2Df& scale
	) const
