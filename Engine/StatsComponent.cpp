...
namespace Engine
{
	StatsComponent::StatsComponent(GameObject* gameObject)
	...
	void StatsComponent::SetStats(float newHealth, float newArmor)
	{
		...
		LOG_INFO(
			gameObject->GetName() +
			" stats initialized. HP: " + std::to_string(health) +
			", Armor: " + std::to_string(armor)
		);
	}

	float StatsComponent::GetHealth() const
	{
		return health;
	}

	float StatsComponent::GetArmor() const
	{
		return armor;
	}

	bool StatsComponent::IsDead() const
	{
		return isDead;
	}
